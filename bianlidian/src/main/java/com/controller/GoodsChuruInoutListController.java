        ...
        if(goodsChuruInoutList !=null){
            ...
            BeanUtils.copyProperties( goodsChuruInoutList , view );//把实体数据重构到view中
                ...
                if(goodsChuruInout != null){
                    BeanUtils.copyProperties( goodsChuruInout , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
...
//                            goodsChuruInoutListEntity.setGoodsChuruInoutId(Integer.valueOf(data.get(0)));   //采购 要改的
//                            goodsChuruInoutListEntity.setGoodsId(Integer.valueOf(data.get(0)));   //商品 要改的
